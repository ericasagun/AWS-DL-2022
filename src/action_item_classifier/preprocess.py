...
def featurize(doc):
    ...
    for idx, token in enumerate(doc):
        if re.match(r"VB.?", token.tag_) is not None:
            ...
            if next_idx < len(doc):
                s_features[
                    f"{FeatureName.FOLLOWING_POS.name}_{doc[next_idx].pos_}"
                ] += 1
                s_features[
                    f"{FeatureName.FOLLOWING_POSTAG.name}_{doc[next_idx].tag_}"
                ] += 1

            if token.head is not token:
                s_features[
                    f"{FeatureName.PARENT_DEP.name}_{token.head.dep_.upper()}"
                ] += 1
